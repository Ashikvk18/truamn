...
}(jQuery);; /*! Bootstrap Carousel Swipe jQuery plugin v1.1 | https://github.com/maaaaark/bcSwipe | MIT License */
! function(t) {
...
jQuery('.gallery_slider').on('slide', function(e) {
    ...
    activeslide.next().css({
        'left': percentage + '%'
    });
    activeslide.prev().css({
        'left': -1 * percentage + '%'
    });
});
jQuery(document).ready(function() {
    jQuery('.carousel').bcSwipe({
        threshold: 50
    });
});
jQuery(document).ready(function() {
    jQuery(':not(.feature-button-container) + .feature-button-container, * > .feature-button-container:first-of-type').each(function() {
        if (jQuery(this).next('.feature-button-container').length > 0) {
            ...
            jQuery(this).nextUntil(':not(.feature-button-container)').addBack().wrapAll(featureButtonsWrapper);
        }
    });
});
